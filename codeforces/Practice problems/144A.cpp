...
void soln()
{
    ll n, m, x, y, z, k, l, sum=0, mult = 1, ans;
    get n;
    vll a(n);
    arrIn(a,n);
    ll j = n, mn=imx, mx = imn, posmx, posmn;
    loop(i,n){
        if(a[--j]>=mx){
            mx = a[j];
            posmx = j+1;
    ...
    if(posmx < posmn) print((posmx-1)+(n-posmn));
    else if(posmx > posmn) print((posmx-1)+(n-posmn)-1);
    else if(posmx==posmn) print(0);
}
int main()
{
